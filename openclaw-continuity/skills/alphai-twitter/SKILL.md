...
从 monitorList / WebSocket 拿到推文后，用以下规则过滤，再进入分析：

### 排除（噪音）
- `type = retweeted` 且推文正文为空 → 纯转发，无原创观点，可忽略
- `type = replied_to` 且正文 < 10字 → 社交回复（"nice"/"LFG"），不含信息
- `type = profile/icon/banner/nick` → 资料变更，非内容推文

### 保留（高质量信号）
- `type = send` → 原创发推（最高价值）
- `type = quoted` → 引用发评，有观点
- `type = replied_to` 且正文 > 30字 → 有实质内容的回复
- `type = retweeted` 且有 `referenced.text` → 转发值得注意，查被转推文

### 加密相关内容识别

分析推文时，优先提取：
- 合约地址（0x开头 / Solana base58地址）→ 自动触发 `query-token-info`
- $TICKER 符号 → 作为 meme 代币候选
- 价格目标 / MC目标 → 接盘人激活条件
- "首发" / "alpha" / "内部信息" 类词汇 → 标记为高优先级信号（但需交叉验证）

### 信号强度分类

| 信号强度 | 条件 | 处理 |
|---------|------|------|
| 🔴 强信号 | 核心 KOL 原创发推 + 含 CA 或 $ticker | 立刻通知老板 |
| 🟡 中信号 | 核心 KOL 引用/长回复 + 含观点 | 加入日志，晨报汇总 |
| ⬜ 弱信号 | 转发/短回复/资料变更 | 静默记录，不主动推送 |
...
4. **过质量过滤层**（见上方过滤规则），排除噪音推文
5. 生成完整的调用代码（含 dex_cookie 认证和 listenKey 续期）
6. 提供监控策略和最佳实践建议
